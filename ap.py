...
from flask_session import Session # class name 
from otp import genotp
from cmail import sendmail
from stoken import endata,dndata
import mysql.connector
mydb=mysql.connector.connect(user='root',password='Jayasri@07',host='localhost',db='snm2426db')
app=Flask(__name__)
app.config['SESSION_TYPE']='filesystem'
Session(app)#intergration
...
def register():
    if request.method=='POST':
        ...
        try:
            ...
            if count_email[0]==0:
                gotp=genotp() #'G6nG5k'
                userdata={'username':userusername,'useremail':useremail,'userpassword':userpassword,'server_otp':gotp}
                subject=f'SNM APP Verification'
                body=f'Use the OTP for Verify:{gotp}'
                sendmail(to=useremail,subject=subject,body=body)
                flash('The OTP has been sent to given mail')
