...
import re
mydb=connection.MySQLConnection(user='root',password='Gafoor',host='localhost',db='sm2456db')
app=Flask(__name__)
...
def forgotpassword():
    if request.method=='POST':
        ...
        try:
            cursor=mydb.cursor(buffered=True)
            cursor.execute('select count(*) from userdata where usermail=%s',[user_email])
...
def newpassword(userdata):
    if request.method=='PUT':
        ...
        try:
            cursor=mydb.cursor(buffered=True)
            cursor.execute('update userdata set userpassword=%s where usermail=%s',[npassword,forgot_email])
            mydb.commit()
            cursor.close()
        except Exception as e:
            print(e)
