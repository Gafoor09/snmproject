...
mydb=mysql.connector.connect(user='root',password='Gafoor',host='localhost',db='sm2456db')
app=Flask(__name__)
...
def register():
    if request.method=='POST':
        ...
        try:
            cursor=mydb.cursor()
            cursor.execute('select count(usermail) from userdata where usermail=%s',[useremail])
    ...
    try:
        ...
        if request.method=='POST':
            ...
            if user_otp==de_otp['server_otp']:
                cursor=mydb.cursor() #it is mysql cursor created using mysqldb connection
                cursor.execute('insert into userdata(username,usermail,userpassword) values(%s,%s,%s)',[de_otp['username'],de_otp['useremail'],de_otp['userpassword']])
                mydb.commit()
                cursor.close()
...
def login():
    if request.method=='POST':
        ...
        try:
            ...
            cursor.execute('select count(usermail) from userdata where usermail=%s',[login_useremail])
            count_email=cursor.fetchone()
        except Exception as e:
            print(e)
            ...
            if count_email[0]==1:
                cursor.execute('select userpassword from userdata where usermail=%s',[login_useremail])
...
def addnotes():
    ...
    if request.method=='POST':
        ...
        try:
            cursor=mydb.cursor(buffered=True)
            cursor.execute('select userid from userdata where usermail=%s',[session.get('user')])
            ...
            if user_id[0]:
                ...
                flash('cpuld not store notes details')
